(() => {
    const BOOTSTRAP_KEY = '__eventArchiveTranslateBootstrap__';
    ...
    const syncSelectorSize = () => {
        ...
        const referenceBtn = navActions
            ? navActions.querySelector('.nav-btn:not(.language-selector), .btn-primary:not(.language-selector)')
            : null;
        if (!referenceBtn) {
        ...
        if (rect.width > 0 && rect.height > 0) {
            ...
            host.style.setProperty('width', width, 'important');
    ...
    const init = () => {
        ...
        loadTranslateScript()
            ...
            .catch(error => {
                console.warn('[EventArchive] Translation bootstrap failed:', error);
            });
    };
