(() => {
    ...
    // 注意: boardContainer 现在是 WGo.js 容器（div），不是 Canvas 元素
    // Canvas 由 WGo.js 库内部管理，或作为降级方案动态创建
    let ctx = null;  // Will be initialized when needed
    let canvas = null;  // Will reference the actual canvas element

    // 获取或创建 Canvas 上下文（用于降级方案）
    function getCanvasContext() {
        if (ctx) return ctx;

        // 查找或创建 Canvas 元素（降级方案）
        let canvasElement = boardContainer.querySelector('canvas');
        if (!canvasElement) {
            // 如果 WGo 正在运行，不要在它的容器里乱加 Canvas，除非是降级模式
            if (typeof WGoAdapter !== 'undefined' && WGoAdapter.isWGoBoardReady()) {
                return null;
            }
            ...
            boardContainer.appendChild(canvasElement);
        }
    ...
    function init() {
        ...
        // 初始化 WGo.js 棋盘（替代原有的 Canvas）
        if (typeof WGo !== 'undefined' && WGo.Board) {
            try {
                // [修复] 必须传入 onBoardClick，否则 UI 层不知道点击了哪里
                WGoAdapter.initWGoBoard(config.size, config.boardStyle, onBoardClick); 
                console.log('✓ WGo.js 棋盘已初始化');
            } catch (e) {
                console.error('WGo.js 初始化失败，降级到 Canvas 模式:', e);
                // 如果 WGo.js 初始化失败，降级到原有的 Canvas 模式
                resizeCanvas();
            }
        } else {
            console.warn('WGo.js 库未加载，使用 Canvas 模式');
            resizeCanvas();
        }

        // 初始化 AI 系统（KataGo Web Worker）
        if (typeof initializeAI !== 'undefined') {
            initializeAI({
                modelSize: '6b',
                difficulty: state.difficulty,
                size: config.size
            }).then(() => {
                console.log('✓ AI 系统已初始化');
            }).catch(error => {
                // 如果只是本地安全策略错误，已经由 AIManager 处理并降级，这里只记录
                console.warn('AI 系统提示:', error.message);
            });
        } else {
            console.log('⚠️  AI Manager 未加载，将使用启发式 AI');
        }
    ...
    function bindEvents() {
        // 棋盘点击事件 - 使用 boardContainer 作为事件目标
        if (boardContainer) {
            boardContainer.addEventListener('click', onBoardClick);
        }

        // 为了安全起见，检查每个 UI 元素是否存在再绑定事件
        if (ui.newGameBtn) ui.newGameBtn.addEventListener('click', handleNewGameClick);
        if (ui.undoBtn) ui.undoBtn.addEventListener('click', undoMove);
        if (ui.passBtn) ui.passBtn.addEventListener('click', handlePassAction);
        if (ui.hintBtn) ui.hintBtn.addEventListener('click', getHint);
        if (ui.resignBtn) ui.resignBtn.addEventListener('click', resignGame);
        if (ui.joinRoomBtn) ui.joinRoomBtn.addEventListener('click', () => joinRoom(false));
        if (ui.watchRoomBtn) ui.watchRoomBtn.addEventListener('click', () => joinRoom(true));
        ...
        if (ui.boardSizeSelect) {
            ui.boardSizeSelect.addEventListener('change', (e) => {
                ...
                // 重新初始化 WGo.js 棋盘（如果已加载）
                if (typeof WGoAdapter !== 'undefined' && WGoAdapter.isWGoBoardReady()) {
                    WGoAdapter.destroyWGoBoard();
                    // [修复] 传入 onBoardClick
                    WGoAdapter.initWGoBoard(size, config.boardStyle, onBoardClick);
                }
        ...
        const applyBoardDimension = (input) => {
            ...
            if (Number.isNaN(size)) { input.value = config.boardCssSize; return; }
            const nextSize = Math.min(900, Math.max(240, size));
            if (nextSize !== size) input.value = nextSize;
            if (nextSize === config.boardCssSize) return;
        ...
        if (ui.boardDimensionInput) {
            ui.boardDimensionInput.addEventListener('keydown', (e) => {
                if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
                    ...
                    const step = 10;
                    const newSize = e.key === 'ArrowUp' ? Math.min(900, currentSize + step) : Math.max(240, currentSize - step);
                    if (newSize !== currentSize) { e.target.value = newSize; applyBoardDimension(e.target); }
                }
            });
        }
        if (ui.boardDimensionUp) {
            ui.boardDimensionUp.addEventListener('click', () => {
                const current = Number.parseInt(ui.boardDimensionInput.value, 10) || config.boardCssSize;
                const next = Math.min(900, current + 10);
                if (next !== current) { ui.boardDimensionInput.value = next; applyBoardDimension(ui.boardDimensionInput); }
            });
        }
        if (ui.boardDimensionDown) {
            ui.boardDimensionDown.addEventListener('click', () => {
                const current = Number.parseInt(ui.boardDimensionInput.value, 10) || config.boardCssSize;
                const next = Math.max(240, current - 10);
                if (next !== current) { ui.boardDimensionInput.value = next; applyBoardDimension(ui.boardDimensionInput); }
            });
        }

        if (ui.boardStyleSelect) {
            ui.boardStyleSelect.addEventListener('change', (e) => {
                const style = e.target.value;
                if (!boardStyleLabel[style]) return;
                config.boardStyle = style;
                if (typeof WGoAdapter !== 'undefined' && WGoAdapter.isWGoBoardReady()) {
                    WGoAdapter.changeBoardTheme(style);
                }
                draw();
    ...
    function serializeGameState(useSGF = false) {
        // 使用 SGF 格式序列化（Phase 3）
        if (useSGF && typeof serializeGameToSGF === 'function') {
            return serializeGameToSGF(state.history, config.size, {
                komi: config.komi,
                scoringMethod: config.scoringMethod,
                koRule: config.koRule,
    ...
    function applyRemoteGameState(game) {
        ...
        // 如果是 SGF 格式（字符串以 (; 开头），转换为游戏状态
        if (typeof game === 'string' && game.trim().startsWith('(;')) {
            try {
                gameState = sgfToGameState(game, config.size);
                console.log('从 SGF 解析游戏状态:', gameState);
            } catch (error) {
        ...
        // 验证步数（防止旧数据覆盖新数据）
        if (typeof gameState.moveCount === 'number') {
            if (gameState.moveCount < state.moveCount) {
                console.warn('收到旧的步数，忽略', gameState.moveCount, state.moveCount);
                return;
            }
        }

        console.log('应用远程状态:', gameState);
        applyGameState(gameState);

        // 更新最后更新时间用于后续参考
        if (gameState && gameState.updatedAt) {
            state.lastGameUpdate = gameState.updatedAt;
        }
    ...
    function onBoardClick(event) {
        ...
        if (state.mode === 'online') {
            if (state.role !== 'player' || (state.isHost && !state.guestId) || state.current !== state.human) return;
        } else if (state.current !== state.human) return;

        let point = null;

        // 1. 优先处理 WGo 转发的事件 (来自 ui-adapter.js)
        if (event && event.isWGoEvent && event.coord) {
            point = event.coord;
        } else {
            // 2. [核心修复] 如果 WGo 已接管，必须忽略原生 DOM 点击
            // 否则会因 render.cell 为 0 导致计算出 NaN，引发崩溃
            if (typeof WGoAdapter !== 'undefined' && WGoAdapter.isWGoBoardReady()) {
                return;
            }
        ...
        if (!point || typeof point.x !== 'number' || typeof point.y !== 'number' || isNaN(point.x) || isNaN(point.y)) return;
        if (point.x < 0 || point.x >= config.size || point.y < 0 || point.y >= config.size) return;
        
        // 检查是否已有子
        if (state.board[point.y][point.x] !== 0) {
            const group = getGroup(state.board, point.x, point.y);
            setStatus(`该位置已有棋子，该组剩 ${group.liberties.size} 气。`, 'error');
            return;
        }

        // 劫规则检查
        if (state.koPoint && state.koPoint.x === point.x && state.koPoint.y === point.y) {
            setStatus('劫规则限制：不能立即回提。', 'error');
            return;
        }

        const move = simulateMove(state.board, point.x, point.y, state.human, state.koPoint);
        if (!move) {
            if (isSuicideMove(state.board, point.x, point.y, state.human)) setStatus('禁止自杀性走法。', 'error');
            else setStatus('该位置无法落子。', 'error');
            return;
        }

        applyMove(move, state.human);
        if (state.mode === 'online') { broadcastGameState(); setOnlineTurnStatus(); }
        else { setStatus('AI 思考中...', 'success'); scheduleAiMove(); }
    }
    ...
    async function aiMove() {
        if (state.mode === 'online' || state.gameOver || state.current !== state.ai) {
            return;
        }
        ...
        try {
            // 尝试使用 AI Manager (KataGo Web Worker)
            const aiManager = typeof getAIManager !== 'undefined' ? getAIManager() : null;
            ...
            if (aiManager && aiManager.isReady) {
                try {
                    // 使用 KataGo AI
                    const timeLimit = state.difficulty === 'easy' ? 1000 :
                                     state.difficulty === 'medium' ? 3000 : 5000;

                    const aiResult = await aiManager.genmove(state.board, state.ai, {
                        timeLimit: timeLimit,
                        difficulty: state.difficulty
                    });

                    // 解析 SGF 坐标到棋盘坐标
                    if (aiResult.move === 'pass') {
                        handlePass(state.ai);
                        state.busy = false;
                        return;
                    }
                    ...
                    if (sgfCoord.length >= 2) {
                        ...
                        if (x >= 0 && x < config.size && y >= 0 && y < config.size) {
                            move = simulateMove(state.board, x, y, state.ai, state.koPoint);

                            // 显示 AI 的置信度
                            if (aiResult.confidence) {
                                console.log(`[AI] 置信度: ${aiResult.confidence.toFixed(1)}%, 胜率: ${aiResult.winRate.toFixed(1)}%`);
                            }
                        }
                    }
                } catch (error) {
                    console.warn('KataGo AI 失败，降级到启发式 AI:', error);
                    // 如果 KataGo 失败，回退到启发式 AI
                    const moves = getLegalMoves(state.board, state.ai, state.koPoint);
                    if (moves.length > 0) {
                        move = chooseAiMoveFallback(moves);
                    }
                }
            } else {
                // AI Manager 不可用，使用启发式 AI
                const moves = getLegalMoves(state.board, state.ai, state.koPoint);
                if (moves.length > 0) {
                    move = chooseAiMoveFallback(moves);
                }
     ...
     * 当 KataGo 不可用时使用
     */
    function chooseAiMoveFallback(moves) {
    ...
    function resizeCanvas() {
        ...
        // 尝试使用 WGo.js 调整棋盘大小
        if (typeof WGoAdapter !== 'undefined' && WGoAdapter.isWGoBoardReady()) {
            WGoAdapter.resizeBoardToWGo(cssSize);
            return; // [注意] WGo 模式下直接返回，不更新 render 对象
        }

        // 降级方案：使用原有的 Canvas
        const canvasContext = getCanvasContext();
        if (!canvasContext) return; // 安全检查
    ...
    function getPointFromEvent(event) {
        // 如果 render.cell 是 0 (WGo 模式下)，这里的计算会导致 NaN
        if (!render.cell) return null;
    ...
    function draw() {
        ...
        if (typeof WGoAdapter !== 'undefined' && WGoAdapter.isWGoBoardReady()) {
            try {
                // 同步棋盘状态到 WGo.js
                WGoAdapter.syncBoardToWGo(state.board, state.lastMove);

                // 显示辅助信息
                const showOptions = {
                    liberties: state.showLiberties,
                    eyes: state.showEyes,
                    connections: state.showConnections,
                    influence: state.showInfluence
                };
                WGoAdapter.showBoardAssistance(showOptions, state.board);

                return;
            } catch (e) {
                console.warn('WGo.js 绘制失败，降级到 Canvas:', e);
            }
        }
        ...
        drawBoard();
        if (state.showInfluence) drawInfluence();
        if (state.showConnections) drawConnections();
        drawStones();
        if (state.showEyes) drawEyes();
        if (state.showLiberties) drawLiberties();
        drawLastMove();
        if (state.handAnimation) drawHandAnimation();
    }
    ...
    function startHandAnimation(x, y, color) {
        // [修复] 如果 WGo 正在运行，强制禁用手部动画，防止清空棋盘导致棋子消失
        if (typeof WGoAdapter !== 'undefined' && WGoAdapter.isWGoBoardReady()) {
            return false;
        }
